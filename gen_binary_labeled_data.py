...
FINE_THRESHOLD = 8.0            # Fine search confirmation threshold: 6σ (adjustable: 4-10σ)
...
def main():
    file_path = "IQ_labeled_data/June_downsampling_10ms/"
    dataset_names = ["train_notrevin_x.npy", "val_notrevin_x.npy", "test_notrevin_x.npy"]
    # dataset_names = ["train_notrevin_x.npy"]
    workers = 4
    num_prns = len(PRN_LIST)
    total_binary_classes = []
    for dataset_name in dataset_names:
        ...
        for iq_data in tqdm(iq_complex):
            ...
            confirmed_sats = [r['prn'] for r in final_results if r['confirmed']]
            # if confirmed_sats:
            ...
            # Multi-label target for all PRNs (index 0 -> PRN1, ..., index 31 -> PRN32)
            sample_labels = np.zeros(num_prns, dtype=np.uint8)
            for prn in confirmed_sats:
                if 1 <= prn <= num_prns:
                    sample_labels[prn - 1] = 1

            binary_classes.append(sample_labels)
            total_binary_classes.append(sample_labels)
            # for result in final_results:
                # if result['confirmed']:
                    # print(f"prn:{result['prn']}, fine snr:{result['fine_snr']}, coarse snr:{result['coarse_snr']}")
            #     if result['prn'] == PRN:
            #         stats.append(result['prn'])
            #         SNRs.append(result['fine_snr'])
            #         c = 1 if result['fine_snr'] > 6 else 0
            #         binary_classes.append(c)
                # total_SNRs.append(result['fine_snr'])
        binary_classes = np.asarray(binary_classes, dtype=np.uint8)
        print(f"dataset len:{len(binary_classes)}, labels shape:{binary_classes.shape}")
        # np.savez(file_path+dataset_name.split('.')[0]+"_binary_labels.npz", stats=stats, SNRs=SNRs, binary_classes=binary_classes)
        np.savez(file_path+dataset_name.split('.')[0]+"_binary_labels.npz", binary_classes=binary_classes)
    ...
    total_binary_classes = np.asarray(total_binary_classes, dtype=np.uint8)
    if total_binary_classes.size == 0:
        print("No samples processed, skip ratio summary.")
        return

    print("\nPer-PRN Positive/Negative Ratio Summary (all datasets combined):")
    print("-" * 70)
    print(f"{'PRN':>4} {'Positive':>10} {'Negative':>10} {'Pos Ratio':>10} {'Neg Ratio':>10}")
    print("-" * 70)

    total_samples = total_binary_classes.shape[0]
    for prn_idx, prn in enumerate(PRN_LIST):
        positives = int(total_binary_classes[:, prn_idx].sum())
        negatives = int(total_samples - positives)
        pos_ratio = positives / total_samples
        neg_ratio = negatives / total_samples
        print(f"{prn:>4} {positives:>10} {negatives:>10} {pos_ratio:>10.4f} {neg_ratio:>10.4f}")
